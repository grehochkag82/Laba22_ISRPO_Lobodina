export class Calculator{
    ...
    init() {
        document.querySelectorAll("button").forEach((bitton)=>{
            buttons.addEventListener("click", ()=>{
                this.handleButtonClick(buttons.textContent);
            });
        });
        this.updateDisplay();
    }
...
safeEvaluate(expr){
    if (!this.isValidExpession(expr)){
        throw new Error("Некореектное выражение");
    } return eval(expr);
}
